      ...
      <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-8">
        ...
        <Tabs defaultValue="general" orientation="vertical" className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <TabsContent value="team" className="space-y-6">
            ...
            <div className="grid gap-5 xl:grid-cols-2">{config.team.members.map((member,index) => <Section key={index} title={`${String(index + 1).padStart(2,"0")} · ${member.title}`}><Field label="Роль или имя" value={member.title} onChange={(v) => setValue(["team","members",index,"title"], v)} /><Field label="Регалии" value={member.credentials ?? ""} multiline onChange={(v) => setValue(["team","members",index,"credentials"], v)} /><Field label="Описание" value={member.description} multiline onChange={(v) => setValue(["team","members",index,"description"], v)} /><ImageField label="Фото сотрудника" image={member.image} onChange={(v) => setValue(["team","members",index,"image"], v)} /></Section>)}</div>
          </TabsContent>
